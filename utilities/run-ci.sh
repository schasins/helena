...
SCRIPT_PARAMS="foo=bar,baz=quux"
...
docker run -t -p 5901:5900 -e HELENA_SERVER_URL=$SERVER_URL -e ROW_BATCH_SIZE=1 -e HELENA_PROGRAM_ID=$PROGRAM_ID -e HELENA_RUN_ID=$RUN_ID -e TIME_LIMIT_IN_HOURS=23 -e NUM_RUNS_ALLOWED_PER_WORKER=1 -e DEBUG=1 -e SCRIPT_PARAMS=${SCRIPT_PARAMS} --user apps --privileged helena:latest
